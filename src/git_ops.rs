...
fn list_worktree_entries() -> Result<Vec<WorktreeEntry>> {
    let out = run_git(&["worktree", "list", "--porcelain"])?;
    Ok(parse_worktree_entries(&out))
}

fn has_existing_linked_worktrees(entries: &[WorktreeEntry]) -> bool {
...
fn find_worktree_for_branch(entries: &[WorktreeEntry], branch: &str) -> Option<PathBuf> {
    entries
        .iter()
...
fn switch_to_dir(path: &Path) -> Result<()> {
    env::set_current_dir(path)
        .with_context(|| format!("failed to switch to worktree directory '{}'", path.display()))
}

fn ensure_worktree_for_existing_branch(branch: &str, entries: &[WorktreeEntry]) -> Result<PathBuf> {
    ...
    fs::create_dir_all(&base)
        .with_context(|| format!("failed to create inferred worktree base '{}'", base.display()))?;
...
fn ensure_worktree_for_branch_reset(
    branch: &str,
    at: &str,
...
pub fn checkout(branch: &str) -> Result<()> {
    ...
    if has_existing_linked_worktrees(&entries) {
        let path = ensure_worktree_for_existing_branch(branch, &entries)?;
        switch_to_dir(&path)?;
    }
...
pub fn checkout_new_or_reset(branch: &str, at: &str) -> Result<()> {
    let entries = list_worktree_entries()?;
    if has_existing_linked_worktrees(&entries) {
        let path = ensure_worktree_for_branch_reset(branch, at, &entries)?;
        switch_to_dir(&path)?;
    }
